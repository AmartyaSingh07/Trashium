...
- **Aesthetics & Motion**: Follow the warm earth-toned palette (Bark: `#2A2218`, Sage: `#8FA37E`, Terra: `#C2703D`, Linen: `#F4EFE3`) and tactile feel (WebGL Ribbons + grain overlay) defined in `app/globals.css` (the CSS custom properties there are the source of truth for all tokens). All micro-animations must support accessibility overrides (`prefers-reduced-motion`).
