    ...
    <div className="min-h-screen bg-[#F4EFE3] text-[#2A2218] font-dm p-4 sm:p-6 lg:p-8 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10 mt-4">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="t-glass-card rounded-3xl p-6 bg-[#EDE5D8]/40 border border-[rgba(194,112,61,0.18)] backdrop-blur-md shadow-md flex flex-col items-center text-center">
            ...
            <div className="relative w-24 h-24 flex items-center justify-center mb-4 mt-2">
              <svg className="absolute inset-0 transform -rotate-90 w-full h-full" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="40" stroke="rgba(194,112,61,0.1)" strokeWidth="6" fill="transparent" />
                <circle 
                  cx="50" cy="50" r="40" 
                  stroke="#8FA37E" strokeWidth="6" fill="transparent" 
            ...
            <span className="font-syne font-bold text-lg text-[#2A2218]">{fullName || "Citizen Ledger User"}</span>
          ...
          <div className="t-glass-card rounded-2xl p-5 bg-[#EDE5D8]/20 border border-[rgba(194,112,61,0.12)] flex flex-col gap-4">
            ...
            {[
              ...
              <div key={idx} className="flex justify-between items-center bg-[#F4EFE3]/60 p-3 rounded-xl border border-[#D4C5B0]/30">
                ...
                <span className="font-mono text-xs font-bold text-[#2A2218]">{stat.val}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-8">
          <form onSubmit={handleSaveProfile} className="t-glass-card rounded-3xl p-6 sm:p-8 bg-[#EDE5D8]/30 border border-[rgba(194,112,61,0.15)] backdrop-blur-md shadow-sm flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  disabled={!isEditMode}
                  className="w-full p-3 bg-[#F4EFE3] border border-[#D4C5B0] rounded-xl text-xs text-[#2A2218] focus:outline-none focus:border-[#C2703D] transition-colors disabled:opacity-60"
                  required
                />
              </div>

              <div className="flex flex-col gap-1.5">
                <label className="text-[11px] font-bold uppercase text-[#2A2218] tracking-wider">Registered Authentication Email</label>
                <input
                  type="email"
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[11px] font-bold uppercase text-[#2A2218] tracking-wider">Primary Mobile Vector (Auth Metadata)</label>
                <input
                  type="tel"
                  value={phone}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[11px] font-bold uppercase text-[#2A2218] tracking-wider">Default Operational Sector Hub</label>
                <select
                  value={zone}
                  onChange={(e) => setZone(e.target.value)}
                  disabled={!isEditMode}
                  className="w-full p-3 bg-[#F4EFE3] border border-[#D4C5B0] rounded-xl text-xs text-[#2A2218] focus:outline-none focus:border-[#C2703D] transition-colors disabled:opacity-60 appearance-none"
                >
            ...
            {isEditMode && (
              <div className="mt-4 pt-4 border-t border-[rgba(194,112,61,0.08)] flex justify-end">
                <button
                  type="submit"
                  disabled={loading}
        ...
        <div className="t-glass-card rounded-3xl p-6 bg-[#EDE5D8]/30 border border-[rgba(194,112,61,0.15)] backdrop-blur-md shadow-sm col-span-1 lg:col-span-12 mt-8 animate-fadeIn">
          <div>
            <span className="font-syne font-bold text-xs uppercase tracking-widest text-[#8FA37E] block mb-1">Ecosystem Checkpoints</span>
            <h2 className="font-syne font-bold text-xl text-[#2A2218] tracking-tight">🏆 Environmental Achievements & Badge Log</h2>
          ...
          <Stagger className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {badges.map((badge) => {
              ...
              const cardStyle = earned
                ? "bg-[#EDE5D8]/50 border-[#8FA37E]/40 shadow-sm"
                : inProgress
                ? "bg-[#EDE5D8]/30 border-[#C2703D]/25"
              ...
              return (
                ...
                >
                  <div className="relative mb-3">
                    ...
                    <div className="w-14 h-14 rounded-full bg-[#F4EFE3]/90 p-2 flex items-center justify-center border border-[#D4C5B0]/40 relative">
                  ...
                  <span className="font-syne font-bold text-xs text-[#2A2218] line-clamp-1">{badge.title}</span>
                  ...
                  <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-48 p-2.5 bg-[#2A2218] text-[#F4EFE3] text-[10px] rounded-xl shadow-xl opacity-0 pointer-events-none group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity duration-200 z-50 text-center font-dm border border-[#C2703D]/20">
                    <span className="font-syne font-bold block mb-1 uppercase tracking-wider text-[#8FA37E]">
                      {earned ? "Earned" : inProgress ? `In progress · ${badge.pct}%` : "Locked"}
                    </span>
                    {badge.description}
                    <div className="absolute top-full left-1/2 transform -translate-x-1/2 border-4 border-transparent border-t-[#2A2218]"></div>
                  </div>
                </div>
              );
            })}
